#!/usr/bin/env python3
"""
Phase 8 Trade-Level Analysis
Tracks individual trades to calculate win rate, avg win/loss per trade.
"""
...
# Configuration
PRED_FILE = 'output/Phase_8_Backtest_Results/test_predictions_corrected.csv'
PRICE_DIR = 'data/NIFTY200_Subset10/raw'
K = 10
REBALANCE_FREQ = 5
...
# Symbol mapping
SYMBOL_MAPPING = {
    'AMARAJABAT': 'ARE&M', 'CADILAHC': 'ZYDUSLIFE', 'CENTURYTEX': 'ABREL',
    'EQUITAS': 'EQUITASBNK', 'GMRINFRA': 'GMRAIRPORT', 'HDFC': 'HDFCBANK',
    'IBULHSGFIN': 'IBULLSLTD', 'INFRATEL': 'INDUSTOWER', 'L&TFH': 'LTF',
    'MCDOWELL-N': 'UNITDSPR', 'MINDTREE': 'LTIM', 'MOTHERSUMI': 'MOTHERSON',
    'NIITTECH': 'COFORGE', 'PEL': 'POONAWALLA', 'PVR': 'PVRINOX',
    'SRTRANSFIN': 'SHRIRAMFIN', 'TATAMOTORS': 'TMPV', 'UJJIVAN': 'UJJIVANSFB',
}

print("=" * 80)
print("PHASE 8 TRADE-LEVEL ANALYSIS")
print("=" * 80)
...
# A "trade" is completed when we exit a position (sell)
# We track: entry_date, exit_date, stock, entry_price, exit_price, shares, P&L

all_trades = []  # Completed trades
open_positions = {}  # {stock: {'entry_date': date, 'entry_price': price, 'shares': n}}

dates = sorted([d for d in pred_df.columns if d in price_data[list(price_data.keys())[0]].index])
...
# Simulate trading
holdings = {}  # Current holdings {stock: shares}
cash = 1_000_000
...
print(f"Largest Win: ₹{winning_trades['net_pnl'].max():,.0f}" if len(winning_trades) > 0 else "N/A")
print(f"Largest Loss: ₹{losing_trades['net_pnl'].min():,.0f}" if len(losing_trades) > 0 else "N/A")
...
print(f"\n4. TOP 5 WINNING TRADES")
print("-" * 40)
top_wins = trades_df.nlargest(5, 'net_pnl')[['stock', 'entry_date', 'exit_date', 'pnl_pct', 'net_pnl']]
print(top_wins.to_string(index=False))

print(f"\n5. TOP 5 LOSING TRADES")
print("-" * 40)
top_losses = trades_df.nsmallest(5, 'net_pnl')[['stock', 'entry_date', 'exit_date', 'pnl_pct', 'net_pnl']]
print(top_losses.to_string(index=False))

# Save trade log
trades_df.to_csv('output/Phase_8_Backtest_Results/trade_log.csv', index=False)
print(f"\n✓ Saved detailed trade log to output/Phase_8_Backtest_Results/trade_log.csv")
