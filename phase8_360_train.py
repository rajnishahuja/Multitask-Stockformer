#!/usr/bin/env python3
"""
Phase 8 Step 6: Alpha360 Training Script

Trains Stockformer model with 360 raw lagged OHLCV features (original paper approach).

Features:
- MAE loss (original paper)
- Regularization (weight_decay=1e-5)
- TopK Precision logging
- Resume from checkpoint
- Early stopping (30 epochs no improvement)
- Max 100 epochs

Usage:
    python phase8_360_train.py --config config/Phase8_NIFTY_Subset10_Alpha360.conf
    python phase8_360_train.py --config config/Phase8_NIFTY_Subset10_Alpha360.conf --resume
"""
...
from torch.cuda.amp import autocast, GradScaler  # AMP for ~50% speedup
...
def train(model, trainXL, trainXH, trainXC, bonus_trainX, trainTE, trainY, trainYL, trainYC, valXL, valXH, valXC, bonus_valX, valTE, valY, valYC, adjgat):
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=args.learning_rate, weight_decay=1e-5)  # L2 regularization kept
    ...
    # AMP GradScaler for mixed precision training (~50% speedup)
    scaler = GradScaler()
    ...
    for epoch in tqdm(range(start_epoch, args.max_epoch + 1)):
        ...
        with tqdm(total=num_batch, desc=f"Epoch {epoch}") as pbar:
            for batch_idx in range(num_batch):
                ...
                with autocast():
                    ...
                    # Phase 8 Step 6: MAE loss (original paper approach) with regularization
        ...
        # Check for MAE improvement (primary model selection)
        if mae < best_mae:
            best_mae = mae
            ...
            epochs_no_improve += 1
            log_string(log, f'Epoch {epoch}: No MAE improvement for {epochs_no_improve} epochs (best: {best_mae:.4f})')
        
        # Also track TopK Precision (for monitoring only, not early stopping)
        ...
        # Early stopping check - based on MAE (original paper approach)
        if epochs_no_improve >= early_stop_patience:
            log_string(log, f'Early stopping triggered after {epoch} epochs (no MAE improvement for {early_stop_patience} epochs)')
            break
